...
if __name__ == "__main__":
    ...
    x0 = 1e3
    T = 1
    sde1 = VESDE(sigma_min=1e-1, sigma_max=100.0, T=T)
    ...
    for i, sde in enumerate([sde1, sde2, sde22, sde3, sde4, sde5]):
        ...
        axs[1, 1].plot(
            t, marginal_samples.std(axis=1), "k-", alpha=0.5, label=r"Empirical $\sigma(t)$"
        )
        axs[1, 1].plot(
            t, marginal_samples.mean(axis=1), "r-", alpha=0.5, label=r"Empirical $\mu(t)$"
        )
