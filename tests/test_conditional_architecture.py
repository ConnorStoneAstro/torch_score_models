...
def test_merging_errors_len_args(Net):
    ...
    with pytest.raises(ValueError) as exc_info:
        net(t, x, *c)
    assert "The network requires 2 additional arguments, but 1 were provided." in str(
        exc_info.value
    )
...
def test_merging_errors_embedding_arg(conditions, Net):
    ...
    if len(condition_type) == 1:
        c = [
            torch.ones(B, 1).long() * 4,
        ]
        with pytest.raises(ValueError) as exc_info:
            net(t, x, *c)
        max_int = condition_embeddings[0] - 1
        assert (
            f"Additional argument 0 must be a long tensor with values between 0 and {max_int} inclusively."
        ...
        assert (
            f"Additional argument 1 must be a long tensor with values between 0 and {max_int} inclusively."
            in str(exc_info.value)
        )
