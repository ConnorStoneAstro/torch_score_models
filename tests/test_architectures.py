...
@pytest.mark.parametrize(
    "nf", [2, 4]
)  # Number of filters needs to be power of 2 when small, or at least divisible by 4, need to debug this
...
def test_unets(D, C, ch_mult, nf, num_res_blocks, attention, Net):
    ...
    t = torch.rand([B])
    model = Net(
        C, dimensions=D, nf=nf, ch_mult=ch_mult, num_res_blocks=num_res_blocks, attention=attention
    )
    out = model(t, x)
...
def test_encoder(D, C, ch_mult, latent_size):
    ...
    t = torch.randn([B])
    model = Encoder(pixels=P, channels=C, dimensions=D, ch_mult=ch_mult, latent_size=latent_size)
    out = model(t, x)
